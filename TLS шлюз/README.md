...
В качестве нагрузочного инструмента используется Yandex-Tank в связке с Jmeter. Yandex-Tank оставлен из за свое простоты сбора метрик а так же из за лёгкого отладки при проблемах и внешнего мониторинга, если необходима более глубокая аналитика результатов.
...
```
Все переменные подписаны в комментарии после #, переменные нужны для более простого запуска теста из консоли, не редактируя значения в файле, пример команды будет в самом конце. 
Рассмотрим порядок и Роли более подробнее.
### Общие параметры
**hosts: taps**
Параметр hosts определяется группу хостов, на которую будут распространяться действия ниже. Файл hosts находится по пути `/etc/ansible/hosts` и в нём перечислены адреса Нагрузочных машин в сети управления.
**gather_facts: true/false**
Модуль. собирающий информацию о удалённой системе для последующего использования переменных, например hostname или версия ОС.
**async_value:** 
...
###  rm_tap_files
Задача текущей роли: удалить старые скрипты запуска TAP, т.к. число скриптов можно будет менять в зависимости от TLS сервера и числа физических интерфейсов.
...
- name: copy .jmx #копирование конфига для джметра
  template:
    src: test.j2
    dest: /root/test.jmx
    mode: 0777
- name: copy yatank_script.sh #копирование конфига для запуска ятанка
  template:
...
```
## Запуск тестов
Запуск тестов производится из директории `/etc/ansible/playbooks`, с помощью команды `ansible-playbook start_yandex_tank_vars.yaml -e "users=xxx rps=xxx addr=xxx http_port=xxx file_name=xxx.html cipher=xxx"` где в переменные можно указать нужные параметры, не заходя в отдельные конфиги. По умолчанию тест длиться 15 минут + через 300 секунд все процессы будут завершены принудительно!
При необходимости можно отдельно запустить отдельные плейбуки:
